...
function setupEventListeners() {
    ...
    if (showAllQaBtn) {
        showAllQaBtn.addEventListener('click', function() {
            const faqItems = document.querySelectorAll('.qa-accordion-header');
...
function initAccordions() {
    document.addEventListener('click', function(e) {
        if (e.target.closest('.qa-accordion-header')) {
            const header = e.target.closest('.qa-accordion-header');
...
function updateCustomerSummary() {
    ...
    document.getElementById('summary-type').textContent = currentCustomerData.customer_type;
...
function displayProposal(proposal) {
    ...
    if (p.payment) {
        ...
        document.getElementById('payment-monthly').textContent = p.payment.simulation?.split('、')[0]?.replace('月々', '') || '未設定';
        document.getElementById('payment-reason').textContent = p.payment.reason || '情報がありません';
        
        const simulation = p.payment.simulation || '';
        const match = simulation.match(/頭金(\S+)、/);
        if (match) {
            document.getElementById('payment-downpayment').textContent = match[1];
        }
        
        const periodMatch = simulation.match(/支払期間(\S+)、/);
        if (periodMatch) {
            document.getElementById('payment-period').textContent = periodMatch[1];
        }
        
        const bonusMatch = simulation.match(/ボーナス時(\S+)×年/);
        if (bonusMatch) {
            document.getElementById('payment-bonus').textContent = `年2回 各${bonusMatch[1]}`;
        }
        
        const residualMatch = simulation.match(/残価設定(\S+)、/);
        if (residualMatch) {
            document.getElementById('payment-residual').textContent = residualMatch[1];
        }
    }
    ...
    if (p.timing) {
        ...
        document.getElementById('timing-period').textContent = p.timing.recommended_date || '未設定';
        document.getElementById('timing-reason').textContent = p.timing.reason || '情報がありません';
    }
    
    // 下取りの情報を表示
    if (p.trade_in) {
        document.getElementById('tradein-car').textContent = currentCustomerData.customer_details.match(/現在の車.*?:.*?(\S+)/)?.[1] || '情報なし';
...
function handleAskQuestion() {
    ...
    .then(data => {
        ...
        if (data.status === 'success') {
            ...
            showNotification(`質問応答に失敗しました: ${data.message}`, 'error');
        }
    })
    .catch(error => {
        toggleLoading(false);
